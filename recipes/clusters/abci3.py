...
def prepare_array_jobs(args: Namespace, command: str) -> tuple[str, list[str], list[Path]]:
    assert args.n_tasks_per_job % args.n_procs_per_job == 0

    n_tasks_per_proc = args.n_tasks_per_job // args.n_procs_per_job
...
if [ "$exit_code" -ne 0 ]; then
    ...
    rm {args.log_filename.with_suffix(".${PBS_ARRAY_INDEX}.lock")}
fi
"""

    qsub_args = ["-l", "select=1"]
    qsub_args += ["-J", f"1-{args.n_tasks}:{args.n_tasks_per_job}"]
    qsub_args += ["-o", str(args.log_filename.with_suffix(".^array_index^.log"))]

    lock_filename_list = []
    for array_idx in range(0, args.n_tasks, args.n_tasks_per_job):
        lock_filename_list.append(args.log_filename.with_suffix(f".{array_idx+1}.lock"))
...
def main():
    ...
    sub_parser.add_argument("--n_tasks_per_job", type=int, default=2048)
    sub_parser.add_argument("--n_procs_per_job", type=int, default=32)

    sub_parser = sub_parsers.add_parser("gpu", parents=[parent_parser])
    sub_parser.add_argument("--n_tasks", type=int)
    sub_parser.add_argument("--n_tasks_per_job", type=int, default=1024)
